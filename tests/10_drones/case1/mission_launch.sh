...
SESSION=$USER
CONFIG_FILE="$MUTAC_WS/tests/10_drones/case1/config.yaml"
RVIZ_FILE="$MUTAC_WS/tests/10_drones/camera10.rviz"
...
tmux send-keys "roslaunch multi_robot_simulator multi_robot_simulator.launch rviz_file:=$RVIZ_FILE all_cameras:=false --wait" C-m
...
tmux new-window -t $SESSION:12 -n 'Execution Monitor Drone 7'
tmux send-keys "roslaunch execution_monitor execution_monitor.launch drone_id:=7 --wait" C-m

tmux new-window -t $SESSION:13 -n 'Execution Monitor Drone 8'
tmux send-keys "roslaunch execution_monitor execution_monitor.launch drone_id:=8 --wait" C-m

tmux new-window -t $SESSION:14 -n 'Execution Monitor Drone 9'
tmux send-keys "roslaunch execution_monitor execution_monitor.launch drone_id:=9 --wait" C-m

tmux new-window -t $SESSION:15 -n 'Execution Monitor Drone 10'
tmux send-keys "roslaunch execution_monitor execution_monitor.launch drone_id:=10 --wait" C-m
